...
def sync_state_from_query_params() -> None:
    ...
    accepted_until = st.query_params.get("accepted_until")
    ...
    if accepted_until:
        try:
            expiry = float(accepted_until)
        except ValueError:
            expiry = 0.0
        st.session_state.accepted_until = expiry
        st.session_state.disclaimer_accepted = expiry > time.time()


def remember_acknowledgement() -> None:
    expiry = time.time() + SESSION_TTL_SECONDS
    st.session_state.disclaimer_accepted = True
    st.session_state.accepted_until = expiry
    st.query_params["accepted_until"] = str(int(expiry))


def acknowledgement_is_current() -> bool:
    if not st.session_state.disclaimer_accepted:
        return False
    expiry = float(st.session_state.get("accepted_until", 0.0))
...
def palette() -> dict[str, str]:
    ...
    return {
        ...
        "muted": "#8B949E" if is_dark else "#5A6478",
        "card": "#1B212C" if is_dark else "#F0F2F6",
        "border": "#2D3748" if is_dark else "#D1D5DB",
        "step": "#1A2535" if is_dark else "#E8F0FB",
    }
...
def apply_style() -> None:
    ...
    st.markdown(
        ...
        .block-container {{ padding-top: 32px !important; max-width: 760px; }}
        ...
        .st-key-nd_navbar div[data-testid="column"] {{ padding: 0 3px !important; }}
        ...
        .cta-wrap div.stButton > button {{ width: 320px !important; height: 56px !important; font-size: 1.1rem !important; }}
        .nd-hero {{ text-align: center; padding: 32px 0 20px; }}
        ...
        .nd-step {{ border-radius: 12px; padding: 18px 22px; display: flex; align-items: flex-start; gap: 16px; }}
        .nd-step-num {{ min-width: 36px; height: 36px; border-radius: 50%; background: {c["step"]}; border: 1px solid #4A90E2; color: #4A90E2; font-size: 0.95rem; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }}
        .nd-step-title {{ font-size: 0.95rem; font-weight: 700; color: {c["text"]}; margin-bottom: 3px; }}
        .nd-step-desc {{ font-size: 0.875rem; line-height: 1.5; }}
        .nd-disclaimer {{ background: {c["card"]}; border: 1px solid {c["border"]}; border-left: 5px solid #4A90E2; border-radius: 12px; padding: 35px 40px; max-width: 620px; margin: 40px auto; color: {c["text"]}; }}
        .nd-disclaimer-title {{ font-size: 1.4rem; font-weight: 700; color: #4A90E2; margin-bottom: 18px; }}
        ...
        .nd-result-label {{ font-size: 0.85rem; font-weight: 700; color: #4A90E2; letter-spacing: 0.1em; margin-bottom: 6px; }}
        .nd-result-name {{ font-size: 2.4rem; font-weight: 800; color: {c["text"]}; margin: 0 0 18px; }}
        .nd-result-divider {{ border: 0; border-top: 1px solid {c["border"]}; margin: 18px 0; }}
        .nd-conf-label {{ font-size: 0.9rem; color: {c["muted"]}; margin-bottom: 6px; }}
        ...
        .nd-page-sub {{ font-size: 0.95rem; margin-bottom: 28px; }}
        @media (max-width: 640px) {{ .nd-hero h1 {{ font-size: 2.4rem; }} .nd-result-name {{ font-size: 1.8rem; }} .st-key-nd_navbar button {{ font-size: 0.78rem !important; padding: 5px 7px !important; }} }}
        </style>
        """,
...
def normalize_predictions(predictions: np.ndarray) -> np.ndarray:
    ...
    if scores.size != len(CLASS_NAMES):
        raise ValueError(f"Expected {len(CLASS_NAMES)} model outputs, got {scores.size}.")
        ...
        <div class="nd-steps">
            ...
            <div class="nd-step">
                ...
                <div>
                    ...
                    <div class="nd-step-desc">The scan is resized and classified across all four categories.</div>
                </div>
            </div>
            <div class="nd-step">
...
def render_model_offline() -> None:
    ...
    detail_html = f"<br><small>{html.escape(details)}</small>" if details else ""
    st.markdown(
        f"""
        <div style="
...
def render_portal() -> None:
    ...
    st.markdown(
        ...
        <div class="nd-result">
            ...
            <div class="nd-result-name">{html.escape(CLASS_NAMES[idx])}</div>
